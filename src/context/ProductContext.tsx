...
interface ProductContextType {
  ...
  addProduct: (name: string, price: number, description: string, image: string, stock: number) => Promise<void>;
  updateProduct: (productId: string, name: string, price: number, description: string, image: string, stock: number) => Promise<void>;
...
export const ProductProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  const addProduct = useCallback(
    async (name: string, price: number, description: string, image: string, stock: number) => {
      if (!user) return;
  ...
  const updateProduct = useCallback(
    async (productId: string, name: string, price: number, description: string, image: string, stock: number) => {
      setLoading(true);
      try {
        const currentProducts = await AsyncStorage.getItem(STORAGE_KEYS.PRODUCTS);
        const allProducts = currentProducts ? JSON.parse(currentProducts) : [];
        ...
        const updatedProducts = allProducts.map((p: Product) =>
          p.id === productId
            ? { ...p, name, price, description, image, stock }
            : p
        );
